"""Protocol implementation for Schellenberg USB Funk-Stick."""
...
@dataclass
class DeviceEvent:
    """Represents an event from a Schellenberg device."""
...
@dataclass
class StickStatus:
    """Represents the status of the USB stick."""

    is_connected: bool = False
    device_version: str | None = None
    device_mode: str | None = None  # boot, initial, or listening
...
class SchellenbergProtocol(asyncio.Protocol):
    ...
    def __init__(
        self, message_callback: Callable[[str], None], connection_lost_callback: Callable[[], None]
    ) -> None:
...
class SchellenbergStick:
    ...
    This is a low-level library that handles the serial protocol.
    It does not depend on Home Assistant.
    """

    port: str
    loop: asyncio.AbstractEventLoop
    on_device_event: Callable[[DeviceEvent], None] | None = None
    on_status_change: Callable[[StickStatus], None] | None = None
    on_pairing_response: Callable[[str], None] | None = None

    _transport: asyncio.Transport | None = field(default=None, init=False, repr=False)
    _protocol: SchellenbergProtocol | None = field(default=None, init=False, repr=False)
    _registered_devices: dict[str, str] = field(default_factory=dict, init=False, repr=False)
    _is_connecting: bool = field(default=False, init=False, repr=False)
    _pairing_future: asyncio.Future[str] | None = field(default=None, init=False, repr=False)
    _stop_pairing_task: asyncio.Task[None] | None = field(default=None, init=False, repr=False)

    # USB stick status
    _is_connected: bool = field(default=False, init=False, repr=False)
    _device_version: str | None = field(default=None, init=False, repr=False)
    _device_mode: str | None = field(default=None, init=False, repr=False)
    _verify_future: asyncio.Future[bool] | None = field(default=None, init=False, repr=False)
    _device_id_future: asyncio.Future[str] | None = field(default=None, init=False, repr=False)
    _hub_id: str | None = field(default=None, init=False, repr=False)

    # Retry queue for commands that failed with "stick busy"
    _pending_retry_command: str | None = field(default=None, init=False, repr=False)
    _retry_task: asyncio.Task[None] | None = field(default=None, init=False, repr=False)

    async def connect(self) -> bool:
        """Establish a connection to the serial port.

        Returns True if connection and verification succeeds.
        """
        if self._is_connecting or (self._transport and not self._transport.is_closing()):
        ...
        try:
            (
                ...
                self.loop,
                lambda: SchellenbergProtocol(self._handle_message, self._on_connection_lost),
                self.port,
                baudrate=112500,
            )
            ...
            if not await self.verify_device():
                _LOGGER.error("Device verification failed - not a Schellenberg USB stick")
                if self._transport:
    ...
    def _notify_device_event(self, event: DeviceEvent) -> None:
        """Notify device event callback."""
    ...
    def _handle_message(self, message: str) -> None:
        ...
        if message == "tE":
            ...
            if self._pending_retry_command:
                ...
                self._retry_task = asyncio.create_task(self._retry_command_after_delay())
            return
        ...
        if message.startswith("sl") and len(message) >= 8:
            ...
            if self._pairing_future and not self._pairing_future.done():
                ...
                self._stop_pairing_task = asyncio.create_task(self._stop_pairing_mode(delay=True))
        ...
        if message.startswith("ss") and len(message) >= 18:
            try:
                ...
                _LOGGER.debug("Parsed: enum=%s, id=%s, cmd=%s", device_enum, device_id, command)
                ...
                self._notify_device_event(event)
    ...
    async def pair_device_and_wait(self) -> tuple[str, str] | None:
        ...
        self._pairing_future = self.loop.create_future()
        ...
        try:
            ...
            device_id = await asyncio.wait_for(self._pairing_future, timeout=PAIRING_TIMEOUT)
    ...
    def register_existing_devices(self, devices: list[dict[str, Any]]) -> None:
        ...
        for device in devices:
            ...
            if device_id and device_enum:
                ...
                _LOGGER.debug("Registered existing device %s with enum %s", device_id, device_enum)
    ...
    def register_entity(self, device_id: str, device_enum: str) -> None:
        ...
        self._registered_devices[device_id] = device_enum
        _LOGGER.debug("Registered entity for device %s with enum %s", device_id, device_enum)
    ...
    async def verify_device(self) -> bool:
        ...
        self._verify_future = self.loop.create_future()
    ...
    async def get_device_id(self) -> str | None:
        ...
        self._device_id_future = self.loop.create_future()
